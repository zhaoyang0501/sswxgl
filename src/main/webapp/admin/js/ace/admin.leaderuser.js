jQuery.adminAdminuser = {
		...
		initSearchDataTable : function() {
			if (this.adminuserDataTable == null) {
				this.adminuserDataTable = $('#dt_adminuser_view').dataTable({
					...
					"bProcessing" : true,
					"bSort" : false,
					"sAjaxSource" : $.ace.getContextPath() + "/admin/leaderuser/list",
		...
		deleteAdminuser :function(id){
			bootbox.confirm( "是否确认删除？", function (result) {
	            if(result){
	            	$.ajax({
	        			type : "get",
	        			url : $.ace.getContextPath() + "/admin/leaderuser/delete?id="+id,
	        			dataType : "json",
		...
		save : function (){
			if($.adminAdminuser.toSave){
				$.ajax({
	    			type : "post",
	    			url : $.ace.getContextPath() + "/admin/leaderuser/save",
	    			data:{
				...
				$.ajax({
	    			type : "post",
	    			url : $.ace.getContextPath() + "/admin/leaderuser/update",
	    			data:{
