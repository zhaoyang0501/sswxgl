jQuery.adminAdminuser = {
		...
		initSearchDataTable : function() {
			if (this.adminuserDataTable == null) {
				this.adminuserDataTable = $('#dt_adminuser_view').dataTable({
					...
					"bProcessing" : true,
					"bSort" : false,
					"sAjaxSource" : $.ace.getContextPath() + "/admin/workuser/list",
		...
		deleteAdminuser :function(id){
			bootbox.confirm( "是否确认删除？", function (result) {
	            if(result){
	            	$.ajax({
	        			type : "get",
	        			url : $.ace.getContextPath() + "/admin/workuser/delete?id="+id,
	        			dataType : "json",
		...
		save : function (){
			if($.adminAdminuser.toSave){
				$.ajax({
	    			type : "post",
	    			url : $.ace.getContextPath() + "/admin/workuser/save",
	    			data:{
				...
				$.ajax({
	    			type : "post",
	    			url : $.ace.getContextPath() + "/admin/workuser/update",
	    			data:{
